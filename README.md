
<div align="center">

![Ryazhenka Logo](assets/Ryazhalogo.png)

[![Telegram](https://img.shields.io/badge/Telegram-@Ryazhenkacfw?style=for-the-badge&logo=telegram&logoColor=white&labelColor=000000)](https://t.me/Ryazhenka) [![Release](https://img.shields.io/badge/Release-latest-blue?style=for-the-badge&logo=github&logoColor=white&labelColor=000000)](https://github.com/Dimasick-git/Ryzhenka/releases/latest) [![License](https://img.shields.io/badge/License-MIT-black?style=for-the-badge)](LICENCE)

</div>

# Ryazhenka

Коротко и по делу.
**Содержание**
- Назначение
- Ключевые компоненты
- Установка
- Конфигурация
- Обновление и релизы
- Формат релизов и changelog
- Контакты и сообщество

---

**Назначение**

Описание: этот репозиторий содержит релизы и инструкции для установки Ryazhenka CFW. Материалы предназначены для опытных пользователей, знакомых с процедурами загрузки кастомной прошивки на Nintendo Switch.

---

**Ключевые компоненты (обязательное выделение)**

- `Ryazhahand-Overlay` — система оверлеев для управления контроллерами и пользовательских настроек геймпада. Краткое назначение: кастомизация назначений кнопок, управление профилями и доступ к быстрым настройкам из оверлея. Разработано: Dimasick-git.
- `Ryazha-clk` — модуль управления частотами CPU/GPU и профилями производительности. Краткое назначение: применение и переключение профилей частот, сохранение пользовательских конфигураций, безопасное изменение параметров управления питанием. Разработано: Dimasick-git.
- `Ryazhenka Tuner` — инструмент тонкой настройки прошивки и системных параметров. Краткое назначение: конфигурирование системных параметров, скрипты автооптимизации и инструменты диагностики. Разработано: Dimasick-git.

Примечание: перечисленные модули являются основными компонентами экосистемы Ryazhenka и должны рассматриваться как первоочередные при настройке системы.

---

**Сводная статистика (агрегированная по репозиториям автора)**

- Общее количество загрузок артефактов релизов по репозиториям `Dimasick-git`: **13094**
- Текущий релиз `Ryzhenka`: **v7.2.0** — см. [releases/tag/v7.2.0](https://github.com/Dimasick-git/Ryzhenka/releases/tag/v7.2.0) (детали в разделе Releases)
---

**Сводная статистика (агрегированная по репозиториям автора)**

- Суммарные загрузки всех релизов: <!--TOTAL_DOWNLOADS-->**11281**<!--/TOTAL_DOWNLOADS-->
- Последний релиз Ryazhenka: [releases/latest](https://github.com/Dimasick-git/Ryzhenka/releases/latest) (тег: v7.2.0)

---

**Основные модули**

| Модуль | Описание | Скачивания | Ссылка |
|---|---|---:|---|
| `Ryzhenka` | Основной пакет прошивки | 11000 | https://github.com/Dimasick-git/Ryzhenka/releases/latest |
| `Ryazhahand-Overlay` | Оверлей управления контроллером (разработано: Dimasick-git) | 281 | https://github.com/Dimasick-git/Ryazhahand-Overlay |
| `Ryazha-clk` | Управление частотами CPU/GPU (разработано: Dimasick-git) | — | https://github.com/Dimasick-git/Sys-clk |
| `Ryazhenka Tuner` | Инструменты настройки и диагностики (разработано: Dimasick-git) | — | https://github.com/Dimasick-git/Ryazhenkabestcfw-Tuner |

---

**Все репозитории автора (динамические шильды загрузок)**

Раздел автоматически отображает динамические шильды загрузок из GitHub через shields.io. Числа обновляются в реальном времени при обращении к странице.

| Репозиторий | Шильды |
|---|---|
| EdiZon | [![downloads](https://img.shields.io/github/downloads/Dimasick-git/EdiZon/total?style=flat-square)](https://github.com/Dimasick-git/EdiZon/releases) [![release](https://img.shields.io/github/v/release/Dimasick-git/EdiZon?style=flat-square)](https://github.com/Dimasick-git/EdiZon/releases/latest) |
| Fizeau | [![downloads](https://img.shields.io/github/downloads/Dimasick-git/Fizeau/total?style=flat-square)](https://github.com/Dimasick-git/Fizeau/releases) [![release](https://img.shields.io/github/v/release/Dimasick-git/Fizeau?style=flat-square)](https://github.com/Dimasick-git/Fizeau/releases/latest) |
| FPSLocker | [![downloads](https://img.shields.io/github/downloads/Dimasick-git/FPSLocker/total?style=flat-square)](https://github.com/Dimasick-git/FPSLocker/releases) [![release](https://img.shields.io/github/v/release/Dimasick-git/FPSLocker?style=flat-square)](https://github.com/Dimasick-git/FPSLocker/releases/latest) |
| Minecraft-Online-for-switch | [![downloads](https://img.shields.io/github/downloads/Dimasick-git/Minecraft-Online-for-switch/total?style=flat-square)](https://github.com/Dimasick-git/Minecraft-Online-for-switch/releases) [![release](https://img.shields.io/github/v/release/Dimasick-git/Minecraft-Online-for-switch?style=flat-square)](https://github.com/Dimasick-git/Minecraft-Online-for-switch/releases/latest) |
| Mission-Control | [![downloads](https://img.shields.io/github/downloads/Dimasick-git/Mission-Control/total?style=flat-square)](https://github.com/Dimasick-git/Mission-Control/releases) [![release](https://img.shields.io/github/v/release/Dimasick-git/Mission-Control?style=flat-square)](https://github.com/Dimasick-git/Mission-Control/releases/latest) |
| ovlSysmodules | [![downloads](https://img.shields.io/github/downloads/Dimasick-git/ovlSysmodules/total?style=flat-square)](https://github.com/Dimasick-git/ovlSysmodules/releases) [![release](https://img.shields.io/github/v/release/Dimasick-git/ovlSysmodules?style=flat-square)](https://github.com/Dimasick-git/ovlSysmodules/releases/latest) |
| PPSSPP | [![downloads](https://img.shields.io/github/downloads/Dimasick-git/PPSSPP/total?style=flat-square)](https://github.com/Dimasick-git/PPSSPP/releases) [![release](https://img.shields.io/github/v/release/Dimasick-git/PPSSPP?style=flat-square)](https://github.com/Dimasick-git/PPSSPP/releases/latest) |
| Ryazha-Cheker | [![downloads](https://img.shields.io/github/downloads/Dimasick-git/Ryazha-Cheker/total?style=flat-square)](https://github.com/Dimasick-git/Ryazha-Cheker/releases) [![release](https://img.shields.io/github/v/release/Dimasick-git/Ryazha-Cheker?style=flat-square)](https://github.com/Dimasick-git/Ryazha-Cheker/releases/latest) |
| Ryazha-Status-Monitor | [![downloads](https://img.shields.io/github/downloads/Dimasick-git/Ryazha-Status-Monitor/total?style=flat-square)](https://github.com/Dimasick-git/Ryazha-Status-Monitor/releases) [![release](https://img.shields.io/github/v/release/Dimasick-git/Ryazha-Status-Monitor?style=flat-square)](https://github.com/Dimasick-git/Ryazha-Status-Monitor/releases/latest) |
| RyazhaAI | [![downloads](https://img.shields.io/github/downloads/Dimasick-git/RyazhaAI/total?style=flat-square)](https://github.com/Dimasick-git/RyazhaAI/releases) [![release](https://img.shields.io/github/v/release/Dimasick-git/RyazhaAI?style=flat-square)](https://github.com/Dimasick-git/RyazhaAI/releases/latest) |
| Ryazhahand-Overlay | [![downloads](https://img.shields.io/github/downloads/Dimasick-git/Ryazhahand-Overlay/total?style=flat-square)](https://github.com/Dimasick-git/Ryazhahand-Overlay/releases) [![release](https://img.shields.io/github/v/release/Dimasick-git/Ryazhahand-Overlay?style=flat-square)](https://github.com/Dimasick-git/Ryazhahand-Overlay/releases/latest) |
| RyazhaTune | [![downloads](https://img.shields.io/github/downloads/Dimasick-git/RyazhaTune/total?style=flat-square)](https://github.com/Dimasick-git/RyazhaTune/releases) [![release](https://img.shields.io/github/v/release/Dimasick-git/RyazhaTune?style=flat-square)](https://github.com/Dimasick-git/RyazhaTune/releases/latest) |
| Ryazhenka-Overlays | [![downloads](https://img.shields.io/github/downloads/Dimasick-git/Ryazhenka-Overlays/total?style=flat-square)](https://github.com/Dimasick-git/Ryazhenka-Overlays/releases) [![release](https://img.shields.io/github/v/release/Dimasick-git/Ryazhenka-Overlays?style=flat-square)](https://github.com/Dimasick-git/Ryazhenka-Overlays/releases/latest) |
| Ryazhenkabestcfw-Tuner | [![downloads](https://img.shields.io/github/downloads/Dimasick-git/Ryazhenkabestcfw-Tuner/total?style=flat-square)](https://github.com/Dimasick-git/Ryazhenkabestcfw-Tuner/releases) [![release](https://img.shields.io/github/v/release/Dimasick-git/Ryazhenkabestcfw-Tuner?style=flat-square)](https://github.com/Dimasick-git/Ryazhenkabestcfw-Tuner/releases/latest) |
| Ryazhenka_Bot | [![downloads](https://img.shields.io/github/downloads/Dimasick-git/Ryazhenka_Bot/total?style=flat-square)](https://github.com/Dimasick-git/Ryazhenka_Bot/releases) [![release](https://img.shields.io/github/v/release/Dimasick-git/Ryazhenka_Bot?style=flat-square)](https://github.com/Dimasick-git/Ryazhenka_Bot/releases/latest) |
| Ryzhenka | [![downloads](https://img.shields.io/github/downloads/Dimasick-git/Ryzhenka/total?style=flat-square)](https://github.com/Dimasick-git/Ryzhenka/releases) [![release](https://img.shields.io/github/v/release/Dimasick-git/Ryzhenka?style=flat-square)](https://github.com/Dimasick-git/Ryzhenka/releases/latest) |
| ryzhenka_brand | [![downloads](https://img.shields.io/github/downloads/Dimasick-git/ryzhenka_brand/total?style=flat-square)](https://github.com/Dimasick-git/ryzhenka_brand/releases) [![release](https://img.shields.io/github/v/release/Dimasick-git/ryzhenka_brand?style=flat-square)](https://github.com/Dimasick-git/ryzhenka_brand/releases/latest) |
| Sys-clk | [![downloads](https://img.shields.io/github/downloads/Dimasick-git/Sys-clk/total?style=flat-square)](https://github.com/Dimasick-git/Sys-clk/releases) [![release](https://img.shields.io/github/v/release/Dimasick-git/Sys-clk?style=flat-square)](https://github.com/Dimasick-git/Sys-clk/releases/latest) |

**Установка (микро-инструкция)**

1. Скопировать содержимое релиза на корень SD-карты.
2. Запустить устройство в RCM и загрузиться через Hekate.
3. Запустить профиль Ryazhenka.

---

Все ссылки и числа отражают актуальное состояние репозиториев автора. Для правок используйте `CONTRIBUTING.md`.
---

**Основные модули и ссылки**

| Модуль | Короткое описание | Ссылка |
|---|---:|---|
| `Ryazhahand-Overlay` | Оверлей для управления контроллером и профилями | https://github.com/Dimasick-git/Ryazhahand-Overlay |
| `Ryazha-clk` | Управление частотами CPU/GPU и профилями производительности | https://github.com/Dimasick-git/Ryazha-clk |
| `Ryazhenka Tuner` | Инструменты тонкой настройки и диагностики системы | https://github.com/Dimasick-git/Ryazhenkabestcfw-Tuner |

---

---

**Зависимости и совместимость**

- Bootloader: Hekate (версия совместима с релизом). Рекомендуется использовать официальные сборки CTCaer.
- Custom Firmware: Atmosphere (совместимые версии). Требуется совместимая версия Atmosphere для корректной работы модулей.
- Sigpatches: требуются для запуска неподписанных компонентов; поставляются отдельно и не включены в этот репозиторий.

---

**Установка (кратко, технически)**

1. Скачайте релиз из раздела Releases этого репозитория.
2. Распакуйте архив на корень SD-карты (FAT32/exFAT в зависимости от конфигурации).
3. Убедитесь, что в SD находятся необходимые файлы Hekate/Atmosphere согласно документации используемой версии bootloader/CFW.
4. Запустите устройство в RCM и запустите Hekate, выберите вариант загрузки с SD, соответствующий Ryazhenka.
5. После начальной загрузки проверьте работоспособность основных модулей: `Ryazhahand-Overlay`, `Ryazha-clk`, `Ryazhenka Tuner`.

Примечание: детальные инструкции по установке и восстановлению находятся в `INSTALL.md`.

---

**Конфигурация**

- Файлы конфигурации модулей расположены в каталоге `config/` релиза или в корне SD-карты согласно структуре конкретного релиза.
- Для `Ryazha-clk` конфигурации профилей сохраняются в формате JSON/YAML (см. документацию модуля). Изменения требуют перезапуска службы модуля или перезагрузки системы.
- Для `Ryazhahand-Overlay` пользовательские профили контроллеров хранятся в отдельном каталоге профилей; переключение выполняется через оверлей или CLI-инструмент при наличии.

---

**Обновление и релизы**

Рекомендации по обновлению:
- Резервная копия текущей SD-карты перед установкой нового релиза.
- Проверка сумм и цифровых подписей релиза (при наличии).
- Чтение файла `CHANGELOG.md` для выявления несовместимых изменений.

Формат релизов (шаблон):
- Заголовок: `vX.Y.Z — краткое описание изменений`
- Разделы: "Изменения", "Исправления", "Известные проблемы", "Инструкции по обновлению"

Пример заметок релиза (технический шаблон):

Changes
- Обновлён `Ryazha-clk`: исправлена логика применения профилей при переходе в спящий режим. (Разработано: Dimasick-git)
- Добавлен режим диагностики в `Ryazhenka Tuner` для сбора логов производительности. (Разработано: Dimasick-git)

Fixes
- Исправлено состояние оверлея при переключении профилей контроллера в `Ryazhahand-Overlay`.

Known issues
- Описание известных ограничений с указанием условий воспроизведения.

Upgrade instructions
- Пошагово: резервная копия → распаковка → проверка прав доступа → загрузка.

---

**Формат CHANGELOG**

- Разделы по версии в порядке убывания (новые версии сверху).
- Для каждой версии указывать: краткое резюме, подробный список изменений (файлы/модули), автор изменений.

Пример записи:

```
v1.2.0
- Added: Ryazha-clk profile manager (Dimasick-git)
- Fixed: overlay handling in Ryazhahand-Overlay (Dimasick-git)
```

---

**Контрибьюторы и роли**

- `@Dimasick-git` — создатель и основной разработчик; автор ключевых модулей (`Ryazhahand-Overlay`, `Ryazha-clk`, `Ryazhenka Tuner`).
- `@Ryazhenka-Helper-1` — помощь в тестировании и вспомогательные скрипты.
- `@Ryazhenka-Helper-01` — поддержка документации и локализаций.

---

**Сообщество**

Подписка на канал для оперативных уведомлений и обсуждений: https://t.me/Ryazhenkacfw

---

**Лицензия**

Проект распространяется под лицензией MIT. Полная версия лицензии в файле `LICENCE`.

---


